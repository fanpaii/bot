...
function renderPluginList(pluginsList) {
    ...
    pluginsList.forEach(plugin => {
        ...
        let hasUpdate = false;
        let marketVersion = '';
        let marketPlugin = null;

        // 检查插件是否在市场上有对应版本
        if (marketPlugins && Array.isArray(marketPlugins)) {
            marketPlugin = marketPlugins.find(p => p.name === plugin.name);
            if (marketPlugin && marketPlugin.version) {
    ...
    document.querySelectorAll('.btn-update').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (marketPlugin) {
                if(confirm(`确定要将插件 ${pluginName} 从 ${plugins.find(p => p.id === pluginId).version || '1.0.0'} 更新到 ${marketVersion} 吗？`)) {
                    installPlugin(marketPlugin);
                }
            } else {
                showToast('无法找到市场插件信息', 'error');
            }
        });
    });
